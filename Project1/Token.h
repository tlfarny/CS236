...
enum TokenType_t {COMMA, PERIOD, Q_MARK, LEFT_PAREN, RIGHT_PAREN, COLON, COLON_DASH, MULTIPLY, ADD, SCHEMES, FACTS, RULES, QUERIES, ID, STRING, COMMENT, UNDEFINED};
...
public:
    ...
    void toString(ofstream& out){
        string output;
        stringFirst(output);
        stringSecond(output);
        stringThird(output);
        out<<"("<<output<<",\""<<info<<"\","<<lineNumber<<")"<<endl;
    }  //1
private:
    TokenType_t type;
    string info;
